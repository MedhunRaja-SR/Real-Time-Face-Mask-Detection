#C:\Users\Admin\AppData\Local\Programs\Python\Python310\facemask
...
col = fg('blue')
print(col+"[INFO] starting video stream...")
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		if label == "No Mask":
			s = "{0:d}{1:s}".format(c4,"People did not Wear their mask properly.")
			SpeakText(s)			
			col = fg('green')
			print(col +"No. of People did not Wear Your Mask Correctly: ",c4)
			#time.sleep(0.3) #...[we Done..]
...
s6 = "oops,No one Wearing their Mask."

SpeakText(s1)

if c3==count:
	SpeakText(s2)
elif c3 < 1 and c4==count:
	SpeakText(s6)
else:
	SpeakText(s3)
if c4 > 0:
